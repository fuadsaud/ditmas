...
local function _35_(client, bufnr)
  local buf_set_keymap_fn
  local function _36_(mode, mapping, target_fn)
    return vim.keymap.set(mode, mapping, target_fn, {buffer = bufnr, noremap = true})
  end
  buf_set_keymap_fn = _36_
  local expand_path_uri
  local function _37_()
    return nfnl_core.str("file://", vim.fn.expand("%:p"))
  end
  expand_path_uri = _37_
  log("Running config.plugins.lsp.clojure/on_attach")
  local execute_command
  local function _38_(command_name, extra_args)
...
local function setup_typescript_tools()
  local function _50_(a, result, ctx, config)
  ...
  return typescript_tools.setup({expose_as_code_action = "all", handlers = {["textDocument/publishDiagnostics"] = _50_}, code_lens = "all"})
end
